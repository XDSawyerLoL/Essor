  ...
  return (
    <main className="public-policy">
      ...
      <article>
        ...
        <section><h2>1. Données conservées sur l’appareil</h2><p>Le prénom choisi, l’avatar, les parcours, les dates, bilans quotidiens, estimations, progression, réglages, agenda et accès fondateur sont conservés dans le stockage local de l’application. Ces données ne sont pas automatiquement publiées dans la communauté ESSOR.</p><p>Les pages libres du journal sont chiffrées localement par AES-GCM. La clé est dérivée du code PIN avec PBKDF2. Le PIN et la clé ne sont jamais transmis à ESSOR. Oublier le PIN ou effacer les données de l’application rend les pages irrécupérables.</p></section>
        ...
        <aside><strong>Avant la diffusion commerciale</strong><p>Une adresse de contact dédiée à la confidentialité et les informations juridiques complètes de l’éditeur doivent encore être ajoutées à cette page. En attendant, les commandes de retrait et de signalement sont intégrées à la communauté ESSOR.</p></aside>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Link className="button primary policy-return" href="/">Revenir dans ESSOR</Link><Link className="button ghost policy-return" href="/communaute">Voir la communauté</Link></div>
      </article>
    </main>
  );
}
